class Toolbox::Image < Toolbox
  ...
  def generate_an_image(image_generation_prompt_s:)
    ...
    return generate_with_rubyllm(image_generation_prompt_s) if Feature.rubyllm?
  ...
  def generate_with_rubyllm(prompt)
     # Uses name "OpenAI" to avoid picking up Groq (also driver: :openai) if the OpenAI service is deleted.
    ...
    context = RubyLLM.context do |config|
      config.openai_api_key = openai_service.effective_token
    end

    image = RubyLLM::Image.paint(
      prompt,
      model: "gpt-image-1",
      provider: :openai,
