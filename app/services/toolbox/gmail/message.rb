class Toolbox::Gmail::Message
  ...
  def body
    str = @message.payload.try(:parts)&.find { |p| p.mimeType == "text/plain" }&.body&.data&.gsub(/-/, "+")&.gsub(/_/, "/")
    str && Base64.decode64(str)
  end

  def body_html
    str = @message.payload.try(:parts)&.find { |p| p.mimeType == "text/html" }&.body&.data&.gsub(/-/, "+")&.gsub(/_/, "/")
    str && Base64.decode64(str)
  end
