...
4. 'bin/rails db:encryption:init' to create random keys for database encryption
5. `bin/rails credentials:edit` (add values from step 3)
6. `bin/rails db:setup` < Note: This will load the sample fixture data into your database
7. `bin/dev` < Starts up all the services
8. Open [http://localhost:3000](http://localhost:3000) and register as a new user.
9. `bin/rails test` and `bin/rails test:system` to run the comprehensive tests
