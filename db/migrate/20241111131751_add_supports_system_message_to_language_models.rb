class AddSupportsSystemMessageToLanguageModels < ActiveRecord::Migration[7.1]
  def change
    add_column :language_models, :supports_system_message, :boolean, default: false

    ActiveRecord::Base.connection.execute <<-END_SQL
      UPDATE
        language_models
      SET supports_system_message = true
      WHERE
