class CreateLanguageModels < ActiveRecord::Migration[7.1]
  ...
  def up
    ...
    [
      ...
      self.create_without_validation!(position: position, name: name, description: description, supports_images: supports_images)
    end
    ...
    (Assistant.all.pluck(:model).uniq - LanguageModel.all.pluck(:name)).each do |model_name|
      ...
      self.create_without_validation!(name: model_name, description: model_name, position: max_position += 1, supports_images: false)
    end
