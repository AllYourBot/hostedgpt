...
class GetNextAIMessageJobGeminiTest < ActiveJob::TestCase
  ...
  test "populates a tool response call from the assistant and creates additional tool messages" do
    ...
    assert_difference "@conversation.messages.reload.length", 2 do
      TestClient::Gemini.stub :function, "helloworld_hi" do
    ...
    assert_equal TestClient::Gemini.thought_signature, @message.content_tool_calls.dig(0, :thought_signature),
      "The signature must be persisted so it can be replayed on the follow-up request"
